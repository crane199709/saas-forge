...
public final class ConsoleAuthenticationExceptionHandler {
    ...
    ResponseEntity<ConsoleProblem> console(ConsoleSessionException error) {
        ...
        return response(status, error.code().name());
    }

    @ExceptionHandler(AuthenticationFailedException.class)
    ResponseEntity<ConsoleProblem> credentials(AuthenticationFailedException error) {
        return response(401, "AUTHENTICATION_FAILED");
    }

    @ExceptionHandler(RefreshRotationInProgressException.class)
    ResponseEntity<ConsoleProblem> rotating(RefreshRotationInProgressException error) {
        return response(409, "REFRESH_ROTATION_IN_PROGRESS");
    }

    @ExceptionHandler({TenantAccessUnavailableException.class, AccessibleMembershipLimitExceededException.class})
    ResponseEntity<ConsoleProblem> contexts(RuntimeException error) { return response(503, "CONTEXTS_UNAVAILABLE"); }
    ...
    ResponseEntity<ConsoleProblem> validation(Exception error) { return response(400, "VALIDATION_FAILED"); }

    @ExceptionHandler(RuntimeException.class)
    ResponseEntity<ConsoleProblem> unavailable(RuntimeException error) {
        org.slf4j.LoggerFactory.getLogger(getClass()).error("Console authentication unavailable: {}", error.getClass().getName());
        return response(503, "SESSION_SECURITY_UNAVAILABLE");
    }

    private static ResponseEntity<ConsoleProblem> response(int status, String code) {
        ...
        return builder.body(problem(status, code));
    }

    static ConsoleProblem problem(int status, String code) {
        return new ConsoleProblem("urn:saas.forge:problem:" + code.toLowerCase(Locale.ROOT).replace('_', '-'),
                ...
                "The Console session request could not be completed.", UUID.randomUUID().toString().replace("-", ""));
    }
